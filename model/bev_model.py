# =============================================================================
# BevModel：Lift-Splat-Shoot（LSS）实现
# 将多路透视图像（4路环视）投影为鸟瞰图（BEV）特征，同时预测像素深度分布
#
# 流程：
#   1. create_frustum()    ← 构建每个像素对应的3D视锥体（depth × H × W 的3D点集）
#   2. get_geometry()      ← 利用内外参把视锥体点投影到自车坐标系
#   3. encoder_forward()   ← CamEncoder提取图像特征+深度，得到每个点的特征向量
#   4. proj_bev_feature()  ← 将3D点特征Splat投影到2D BEV网格（体素累加）
# =============================================================================
import torch
