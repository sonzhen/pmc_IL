# =============================================================================
# ControlPredict：Transformer Decoder 控制指令预测模块
#
# 控制指令被「token化」（参见 dataset/carla_dataset.py::tokenize）：
#   throttle [0,1], brake [0,1], steer [-1,1], reverse {0,1}
#   → 各自映射到 [0, token_nums-4] 范围内的整数 token
#   → 特殊 token：BOS(开始), EOS(结束), PAD(填充)
#
# 训练（forward）：Teacher-forcing
#   输入 gt_control 序列（BOS + 真实tokens），输出下一个token的概率分布
#
# 推理（predict）：自回归
#   从 BOS 开始，每次将已预测的序列送入 Decoder，预测下一个 token
#   重复3次，得到 [throttle, brake, steer] 共3个token（reverse由steer编码中体现）
# =============================================================================
import torch
