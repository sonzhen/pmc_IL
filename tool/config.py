...
# 全局配置模块
#
# Configuration 类：所有超参数的数据类（字段默认为 None，由 get_cfg() 从 YAML 填充）
# get_cfg(cfg_yaml)：解析 config/training.yaml，返回填充好的 Configuration 实例
#
# 关键配置组：
#   训练控制   : epochs, batch_size, learning_rate, weight_decay
#   BEV空间    : bev_x/y/z_bound, d_bound, final_dim, bev_down_sample
#   Transformer: tf_en_* (Encoder), tf_de_* (Decoder)
#   Token化    : token_nums (控制指令离散化粒度)
#   路径       : data_dir, log_dir, checkpoint_dir
# =============================================================================
import os
import torch
