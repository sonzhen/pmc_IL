# =============================================================================
# 【入口3 / 3】在线评估脚本 —— 加载训练好的模型，在 CARLA 中自动泊车并记录指标
#
# 使用方式：python3 carla_parking_eva.py --model_path ./ckpt/last.ckpt
#
# 调用关系：
#   carla_parking_eva.py
#     └── game_loop()
#           ├── NetworkEvaluator (data_generation/network_evaluator.py) ← 评估环境管理
#           │     └── World      (data_generation/world.py)             ← 同数据采集共用
#           ├── ParkingAgent     (agent/parking_agent.py)               ← 推理智能体
#           │     └── ParkingModel (model/parking_model.py)             ← 神经网络推理
#           └── KeyboardControl  (data_generation/keyboard_control.py) ← 可手动干预
#
# 评估循环（每帧）：
#   world_tick → parking_agent.tick（推理→施加控制）→ network_evaluator.tick（判断成功/失败）
#   → render（可视化）→ 保存指标CSV
# =============================================================================
import argparse
import logging
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            network_evaluator.world_tick()   # ① CARLA仿真世界推进一步
            clock.tick_busy_loop(60)
            if controller.parse_events(client, network_evaluator.world, clock):
                return
            parking_agent.tick()             # ② 模型推理：读传感器 → 前向传播 → 施加油门/刹车/方向盘
            network_evaluator.tick(clock)    # ③ 评估判定：检查是否停车成功/碰撞/超时/越界
            network_evaluator.render(display)# ④ 渲染 BEV 画面
            show_control_info(display, parking_agent.get_eva_control(), steer_wheel_img,
                              args.width, args.height, font)  # ⑤ 绘制方向盘/油门/刹车UI
            pygame.display.flip()
