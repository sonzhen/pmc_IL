# =============================================================================
# 【入口2 / 3】模型训练脚本 —— 基于 PyTorch Lightning 的离线训练
#
# 使用方式：python pl_train.py --config ./config/training.yaml
#
# 调用关系：
#   pl_train.py
#     └── train()
#           ├── ParkingTrainingModule (trainer/pl_trainer.py)  ← pl.LightningModule 训练逻辑
#           │     ├── ParkingModel     (model/parking_model.py) ← 核心神经网络
#           │     ├── ControlLoss      (loss/control_loss.py)   ← 控制指令分类损失
#           │     ├── DepthLoss        (loss/depth_loss.py)     ← 深度估计损失
#           │     └── SegmentationLoss (loss/seg_loss.py)       ← BEV分割损失
#           └── ParkingDataModule     (dataset/dataloader.py)  ← pl.LightningDataModule
#                 └── CarlaDataset    (dataset/carla_dataset.py)← 读取磁盘采集的数据
#
# 配置文件：config/training.yaml  → tool/config.py::Configuration
# =============================================================================
import os
import sys
import argparse
