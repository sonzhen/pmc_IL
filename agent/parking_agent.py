...
# 推理智能体：ParkingAgent
#
# 职责：在评估循环中，每帧从 World 读取传感器数据，调用 ParkingModel 进行推理，
#       将预测的控制指令施加到 CARLA 自车上。
#
# 调用关系（由 carla_parking_eva.py 驱动）：
#   ParkingAgent.tick()
#     ├── get_model_data()        ← 传感器数据预处理（crop/normalize/坐标转换）
#     ├── ParkingModel.predict()  ← 前向推理，输出控制token序列
#     ├── detokenize()            ← token → 实际油门/刹车/方向盘/倒档值
#     ├── speed_limit()           ← 安全限速逻辑
#     └── player.apply_control()  ← 施加控制到 CARLA 仿真车辆
#
# 特殊机制：
#   - pre_target_point: 用上一帧预测的BEV分割图中心作为本帧目标点（平滑目标）
#   - process_frequency=3: 每3个仿真步(≈0.1s)推理一次，其余帧复用上次指令
#   - 注意力可视化: 通过 hook 抓取 Transformer 最后一层 self-attention 权重
# =============================================================================
import carla
import math
import pathlib
